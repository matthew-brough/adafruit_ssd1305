...
class BitmapFont:
    ...
    def getbbox(self, text: str) -> tuple[int, int, int, int]:
        ...
        text_width = sum(self.chars.get(c, {}).get('width', self.width) for c in text)
    ...
    def render_text(self, image: Image.Image, position: tuple[int, int], text: str, fill: int = 1) -> None:
        """
